...
function publish(packet, client, cb) {
    if (typeof client !== 'undefined' && typeof packet !== 'undefined') {
        logger.debug('Client ' + client.id + ' Topic ' + packet.topic + ' Payload ' + packet.payload.toString())
    }
    ...
    if (packet.topic === env.topic_ven) {
        ...
        api.updateESPState('ESP Ventilation 1', bstate)
        api.updateESPState('ESP Ventilation 2', bstate)
    }
}
